    ...
    >
      ...
      <div className="flex items-center space-x-2">
        ...
        <div
          ...
          transition-all duration-300 ${isAnimating ? 'bg-white text-[#3E2472]' : 'bg-[#3E2472] text-white'}`}
        >
          {number}
        </div>
        ...
        <h3
          className={`font-bricolage text-[24px] font-semibold leading-[103%] transition-all duration-300 ${isAnimating ? 'text-white' : 'text-[#3E2472]'}`}
        >
          {title}
        </h3>
      </div>
      ...
      <p
        className={`text-[16px] font-gilroy leading-[138%] mt-2 transition-all duration-300 ${isAnimating ? 'text-white' : 'text-[#41335E]'}`}
      >
        {description}
      </p>
      ...
      <motion.div
        className={`absolute inset-0 rounded-[22px] z-[-1] transition-all duration-300 ${isAnimating ? 'bg-[#5B42F3]' : 'bg-white'}`}
      />
    </motion.div>
  );
}